...
ncx_slab_alloc_locked(ncx_slab_pool_t *pool, size_t size)
{
	...
	debug("slab alloc: %uz slot: %ui", size, slot);
...
ncx_slab_stat(ncx_slab_pool_t *pool, ncx_slab_stat_t *stat)
{
	...
	info("total page count : %u",	stat->pages);
	info("free page count  : %u\n",	stat->free_page);
		
	info("small slab use page : %u,\tbytes : %u",	stat->p_small, stat->b_small);	
	info("exact slab use page : %u,\tbytes : %u",	stat->p_exact, stat->b_exact);
	info("big   slab use page : %u,\tbytes : %u",	stat->p_big,   stat->b_big);	
	info("page slab use page  : %u,\tbytes : %u\n",	stat->p_page,  stat->b_page);				

	info("max free pages : %u\n",		stat->max_free_pages);
}
